...
function doEvent.sliderTicks(element, Gui, player_index, textTicks)
  ...
  if element.slider_value ~= tostring(textTicks.text) then
    log("sliderTicks, textTicks is: " .. serpent.block(textTicks))
    textTicks.text = tostring(ticks)
    element.slider_value = ticks -- Set for MP?
  end
  ...
  if element.text and String.is_digit(element.text) then
    ...
    if num then
      ...
      if num ~= sliderTicks.slider_value then
        ...
        element.text = tostring(num)  -- Set this as well, for MP
      end
...
function Guiconfig.gui_event_handler(event, Gui, target)
  ...
  local handler_function = function(evnt)
    ...
    log("Guiconfig.gui_event_handler, player_index: " .. player_index)
    local target_name = player_index .. "_" .. target
    target_element = Gui.elements[target_name] or target
    return event.func(element, Gui, player_index, target_element)
  end
...
function defaults.Ticks(player_index, name, Graphtool)
  local default = {}
  local ticks = Graphtool.config.ticks
  if name == player_index .. "_" .. "textTicks" then
...
local function radiobutton_pair(player_index, name, caption, on_state_name, off_state_name, config_name, tooltip_text, event_func, default_func)
  local element_table = {}
  ...
  local event_function = event_func or function (element, Gui, player_index, target)
    -- log("radiobutton event_func, player_index: " .. player_index)
    -- log("radiobutton event_func, element: " .. serpent.block(element))
    -- log("radiobutton event_func, Gui: " .. serpent.block(Gui))
    if element.state == target.state then
      target.state = not element.state
    end
    local on_element_name = player_index .. "_" .. on_element_name
    log("radiobutton_pair, checking Gui.elements[on_element_name] with on_element_name:" .. on_element_name)
    Gui.Caller.config[config_name] = Gui.elements[on_element_name].state
  end

  local default_function = default_func or function (player_index, elemname, caller)
    local default = {}
    log("default_func, elemname: " ..elemname )
    if elemname == player_index .. "_" .. on_element_name then
...
function Guiconfig.gui_layout(player_index)
  return
    {
              ...
              { type = "flow",                  name = "flowConfig", direction = "vertical",
                                                ...
                                                children =
                {
                  { type = "table",             name = "tableConfigRadio", column_count = 3, direction = "horizontal",
                        ...
                        children =
                    {
                      radiobutton_pair(player_index, "Graphing", {"label.enabled"}, "On", "Off", "enabled", {"tooltip.enabled", {"label.graphtool"}}),
                      radiobutton_pair(player_index, "Separate", {"label.separate"}, "Yes", "No", "separate", {"tooltip.separate", {"label.Yes"}, {"label.No"}}),
                      radiobutton_pair(player_index, "AllowNeg", {"label.allow_neg"}, "Yes", "No", "allow_neg",
